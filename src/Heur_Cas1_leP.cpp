...
Solution Heur_Cas1_leP::solve(Instance t_instance){
    ...
    std::vector<std::vector<std::vector<std::vector<std::vector<int>>>>> Z_sol;  
    Z_sol.resize(1);
                ...
                for (int a=0; a<=1; ++a){
                    if (Zeta[a][1].get(GRB_DoubleAttr_X)>0.5){
                        ...
                        if (variation_temps_cam_d[a] - t_instance.getGraph().getTSPKernelTime(id_node_deb_actuelle,sommets_actuels[nouveau_sommet]) > 0){
                            variation_temps_cam_d[a] = variation_temps_cam_d[a] - t_instance.getGraph().getTSPKernelTime(id_node_deb_actuelle,sommets_actuels[nouveau_sommet]);
                        }
                        else {
                            variation_temps_cam_d[a] = 0;
                        }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        std::cout << std::endl << std::endl << std::endl<< "dernier modèle 3 deplacement" << std::endl << std::endl << std::endl << std::endl;
        ...
        if (nouveau_sommet == sommets_actuels.size()){
            ...
            bool continuer = true;
            while ((nouveau_sommet2<sommets_actuels.size()) && (continuer)){
                    ...
                    for (int eps=0; eps<t_instance.getGraph().getDemand(demandes_actuelles[d]).getAmount() - alt; ++eps){
                        for (int a=0; a<=1; ++a){
                            if (Z[2][a][d][eps][nouveau_sommet2].get(GRB_DoubleAttr_X)>0.5){
                                Z_sol[Z_sol.size()-1][a][demandes_actuelles[d]][quantite_client_satisfait[demandes_actuelles[d]]][sommets_actuels[nouveau_sommet2]] = 1;
                                quantite_client_satisfait[demandes_actuelles[d]] += 1;
                            }
                        }
                    }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (Z_sol.size()<X_sol.size()){
            std::cout << X_sol[X_sol.size()-1] << " : ---" << std::endl << std::endl << std::endl << std::endl;
        }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    std::vector<std::vector<int>> Y_raf;
    Y_raf.resize(T);
    for (int t=0; t<T; ++t){
        Y_raf[t].resize(CardDU);
        for (int d=0; d<CardDU; ++d){
            Y_raf[t][d] = 0;
        }
    }
    std::vector<std::vector<std::vector<int>>> Z_raf;
    Z_raf.resize(T);
    for (int t=0; t<T; ++t){
        Z_raf[t].resize(2);
        for (int a=0; a<=1; ++a){
            Z_raf[t][a].resize(CardDU);
            for (int d=0; d<CardDU; ++d){
                Z_raf[t][a][d] = 0;
            }
        }
    ...
    Z_sol_conv.resize(T);
    for (int t=0; t<T; ++t){
        Z_sol_conv[t].resize(CardDU);
        for (int d=0; d<CardDU; ++d){
            Z_sol_conv[t][d].resize(2);
            for (int a=0; a<=1; ++a){
                Z_sol_conv[t][d][a] = 0;
            }
        }
    ...
    for (int t=0; t<Z_sol.size(); ++t){
        ...
        for (int d=0; d<CardD; ++d){
            ...
            cpt += t_instance.getGraph().getDemand(d).getAmount();
        }
        for (int a=0; a<=1; ++a){
            cpt = 0;
            for (int d=0; d<CardD; ++d){
    ...
    // }
    // name << "flotff";
    // model.addConstr(expff==1,name.str());
    // name.str("");
    ...
    //     }
    //     for (int d=0; d<CardDU; ++d){
    //         exp += Y_raf[t][d] * I.getTruckDeliveryTime();
    //     }
    //     name << "Omega / Y " << t;
    //     model.addConstr(exp<=0,name.str());
    //     name.str("");
